...
// Extracted from notifications.pb.js for PB 0.36 JSVM scope isolation.
...
function getTeamMemberIds(teamIds) {
  ...
  for (var i = 0; i < teamIds.length; i++) {
    ...
    // Also include coach, captain, team_responsible
    try {
      var team = $app.findRecordById("teams", teamIds[i])
      var leaderFields = ["coach", "captain", "team_responsible"]
      for (var lf = 0; lf < leaderFields.length; lf++) {
        var leaders = team.get(leaderFields[lf])
        if (Array.isArray(leaders)) {
          for (var li = 0; li < leaders.length; li++) {
            if (leaders[li]) memberSet[leaders[li]] = true
          }
        } else if (leaders) {
          memberSet[leaders] = true
        }
      }
    } catch (e) {
      // team not found — skip
  ...
  // Also try without season filter as fallback
...
module.exports = {
  ...
  buildPushTitle: buildPushTitle,
  buildPushBody: buildPushBody,
  buildPushUrl: buildPushUrl,
