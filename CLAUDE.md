...
- **2026-03-28** — Directus expand migration + Sentry + Analytics (v2.7.0): Migrated all 62 files from PB `obj.expand?.relation` to Directus inline relations with `fields: ['*', 'relation.*']` and `asObj<T>()` helper. Added Sentry error tracking (@sentry/react with ErrorBoundary, user context, source map uploads via @sentry/vite-plugin). Updated CSP headers for Sentry + analytics. Added public read permission for participations in Directus. Recommended Cloudflare Web Analytics (dashboard toggle, no code). 62 files changed, +1123/-366 lines.
