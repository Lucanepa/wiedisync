...
- **2026-04-20** v3.16.7 — Fix: ParticipationRosterModal response-time only showed for declined members. Directus `date_updated` is only written on UPDATE, not CREATE — first-time confirmed/maybe responses had `date_updated = NULL` while declined members (who often flipped from a prior status) had it populated. Both player + staff rows now fall back to `date_created` when `date_updated` is missing.
- **2026-04-20** v3.16.6 — Security audit part 2: Coach scoping + Sport Admin dangerous-delete restriction + CSP tightening. (a) Migration 026 scopes KSCW Coach writes to teams via `teams.coach` M2M alias on trainings/games/events/slot_claims/task_templates/referee_expenses/scorer_delegations (20 permission rows). Previously `{}` — a coach from team A could modify team B's data via raw `/items/*`. (b) Migration 027 drops `members.delete` + `teams.delete` from Sport Admin (club-wide blast radius actions should be full-admin only); keep create + update. (c) `public/_headers` CSP dropped `https://*.sentry.io` wildcards from `connect-src` — browser doesn't need them since Sentry events go through `sentry-tunnel.kscw.ch`. Deferred: `style-src 'unsafe-inline'` (83 dynamic inline styles), remaining Sport Admin cross-sport writes (needs per-sport policy split).
- **2026-04-20** v3.16.5 — Security audit follow-up. (a) CRITICAL: `{}` row filter on KSCW Member policy for `messages`/`conversations`/`message_reactions`/`reports` let any authenticated member enumerate all DMs + reports via raw `/items/*`. Migration 023 scopes via `conversation_members.member.user=$CURRENT_USER`. Verified with live member token (/items/messages now returns []). (b) Member PII: `email`+`phone` removed from non-self `members` read (row 7045); self-read row (7046) keeps them. Migration 024. (c) Report-flood rate limit: 5/hour/member on POST /kscw/messaging/reports. (d) Soft-delete redaction: `DELETE /kscw/messaging/messages/:id` now nulls `body`+`original_body`; reports row snapshots pre-redaction. (e) Broadcast per-sender global cap: 10/hour/sender in `checkRateLimit()`. (f) Public `feedback.create` dropped `status` from whitelist — migration 025. (g) npm audit fix: Vite 8.0.2→8.0.9 + DOMPurify patched. Deferred: Sport Admin + Coach per-sport/per-team scoping needs M2M alias on `teams` (not defined); CSP `unsafe-inline` kept for now.
- **2026-04-20** v3.16.4 — Full i18n sweep of hardcoded German/Swiss sites (extension of 3.16.2's `formatWeekday` fix). `formatDate`/`formatDateCompact`/`formatTime` in `src/utils/dateHelpers.ts` now also resolve `currentLocale()` (exported); Hallenplan month (was `en-US`), profile + member birthdates, status page, admin ResultsTable/AuditLog/InfraHealth/DataHealth/ExplorePage dates, scorer helpers all now follow `i18n.language`. `VolleyFeedbackPage` fully i18n-ized (was `lang === 'de' ? de : en` only) — 20 new `vf*` keys added to all 5 admin bundles. Stripped 48 German `defaultValue: '...'` fallbacks (DeleteAccountModal, messaging dialogs, AnnouncementsPage, SpielsamstageEditor). Added missing keys: `common.create`, `announcements.linkInvalid`, `announcements.confirmMassEmail` to all 5 locales. Sorting `localeCompare` / `Intl.Collator` now uses `i18n.language` instead of hardcoded `'de'`.
- **2026-04-20** v3.16.3 — Two participation fixes. (1) Own RSVP no longer reflected on `/trainings` + `/games` cards: `useActivitiesWithParticipations` calls `kscwApi` directly, which skips the `fetchItems → stringifyIds` FK-stringification — `p.member === user.id` never matched (integer vs string). Fix: exported `stringifyIds` + applied to both `items` + `participations`. (2) "Show response time" toggle had no effect: `ParticipationRosterModal` still read `participation.updated` (PocketBase name) — renamed to `date_updated`.
- **2026-04-20** v3.16.2 — Fix: weekday abbreviations in home-page appointments list now follow app language. `formatWeekday()` in `src/utils/dateHelpers.ts` was hardcoded to `de-CH`; new `currentLocale()` helper reads `i18n.language` (gsw → de-CH, en → en-GB to keep dd/mm + 24h); resolved automatically — no call-site changes. Dates + times still Swiss-format by design.
- **2026-04-20** v3.16.1 — Fix: consolidated ConversationPage loading (initialized `useConversation.isLoading` + `useConversationMembers.loading` to `true` so the first render doesn't look "ready with 0 items"; `ThreadView` now gates the full UI on `!isLoading && (!needsMembers || !membersLoading)` so header + thread + composer appear together). Fix: admin `new_report` notifications now route to `/admin/reports` (`NotificationPanel.getNavigationPath` added the `new_report` / `report` case); `Notification` type extended with `'new_report'` and `'report'`.
