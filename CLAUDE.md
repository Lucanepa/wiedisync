...
- **Capitalisation**: All user-facing strings (UI labels, button text, badges, toasts, push/email subjects, i18n values, error messages, placeholder text, form field labels, etc.) MUST start with a capital letter — Sentence case for full sentences, Title Case for short labels. Lowercase is only acceptable when explicitly required (e.g. all-caps acronyms, brand names with documented lowercase styling like `iOS`, code identifiers in inline `<code>`). Applies to all 5 locales (en/de/gsw/fr/it). Example violations to fix on sight: "absent", "training cancelled", "log in", "save". Correct: "Absent", "Training cancelled", "Log in", "Save".
