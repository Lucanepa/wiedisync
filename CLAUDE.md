...
- **2026-04-20** v3.16.3 — Two participation fixes. (1) Own RSVP no longer reflected on `/trainings` + `/games` cards: `useActivitiesWithParticipations` calls `kscwApi` directly, which skips the `fetchItems → stringifyIds` FK-stringification the rest of the app relies on. `p.member` came back as integer `8` while `user.id` was `"8"`, so `p.member === user.id` in `TrainingsPage` + `GamesPage` never matched → `myParticipation === undefined` → grey buttons, no left banner. Fix: exported `stringifyIds` from `src/lib/api.ts` and apply it to both `items` + `participations` inside `useActivitiesWithParticipations`. (2) "Show response time" toggle had no effect because `ParticipationRosterModal` still read `participation.updated` / `sp.updated` (PocketBase field name) — renamed to `date_updated` (which the endpoint's `DEFAULT_PARTICIPATION_FIELDS` already returns).
