  ...
  ({ variant = 'primary', size = 'md', loading, icon, className = '', disabled, children, ...props }, ref) => {
    return (
      <button
        ...
        className={`inline-flex min-h-[44px] items-center justify-center rounded-lg font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-brand-500/40 disabled:opacity-50 disabled:cursor-not-allowed sm:min-h-[40px] ${variants[variant]} ${sizes[size]} ${className}`}
        {...props}
      >
