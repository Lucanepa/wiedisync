...
- **2026-05-10** v4.6.1 — Explicit close affordance on bottom sheets. `MoreSheet` and `NotificationPanel` get a chevron-down button at the top-right of the handle row on mobile (44×44px touch target) → `startClose()`. NotificationPanel also gets an `X` button in its desktop header (`lg:` only). Same animation path as backdrop tap. Reason: the v4.6.0 scroll fix made content reachable but dismissing the sheet on a phone still required a small backdrop tap above the sheet OR a nav-link tap (which navigates). The chevron makes "just close it" a one-tap action.
- **2026-05-10** v4.6.0 — Roster export (CSV / PNG / PDF) + mobile bottom-sheet scroll fix (Android + iOS) + roster filter dropdown. New Export dropdown in `ParticipationRosterModal`, gated by `canEditRoster`. Respects the active status filter; "All" also appends waitlist + staff. Columns: name (with Coach/C/TR/Staff suffix), jersey number (`members.number`), default positions (`members.position[]`), status (incl. declined-by-absence variant), guests, note (or absence reason), RSVP timestamp. CSV via existing `toCSV`/`downloadText` helpers + UTF-8 BOM for Excel umlaut handling. PNG via `html-to-image`, PDF via `html-to-image` → `jspdf` with multi-page A4 slicing. Both libs are dynamic-imported (main bundle unchanged: 819KB gzip → 819KB gzip). Hidden printable view rendered off-screen at 800px wide with light-mode inline styles so exports are dark-mode-independent. Filename pattern: `<title>_<date>_<filter>.<ext>`. Position summary above the table — pill row with `<count> <label>` per declared position in the current population (members counted once per position they hold, e.g. setter/outside hybrid contributes to both); fixed order S→O→M→D→L→BB equivalents→guest→other; CSV gets a `Positions:` metadata line. Mobile scroll fix: `MoreSheet` and `NotificationPanel` couldn't be scrolled when content overflowed 85vh — both placed `animate-sheet-up` (translateY) and `overflow-y-auto` on the same DOM node. Active/settled `transform` promotes the element to a compositor layer; both Android Chrome and iOS Safari mishandle touch-scroll on that layer, and `position: sticky` children of a transformed ancestor are broken on both. Restructured to outer animated wrapper (`flex flex-col max-h-[85vh]` + `animate-sheet-*`) wrapping inner `flex-1 overflow-y-auto overscroll-contain`. `panelRef` moved to the new scroll container. `onAnimationEnd` hardened (`e.target === e.currentTarget`). `Modal` (Vaul Drawer) confirmed fine. Bonus: filter chip strip → single `DropdownMenu`.
